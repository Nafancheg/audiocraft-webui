...
function makeAudioElement(json_data, filename, use_reverse_ordering) {
    ...
    if (key === 'seed' || key === 'cfg_seed' || key === 'sid') { // accept legacy sid
            paramDiv.textContent = `Seed: ${json_data.parameters[key]}`;
        } else {
    ...
    delBtn.className = 'delete-audio';
    delBtn.textContent = 'X';
    delBtn.title = 'Удалить';
    delBtn.addEventListener('click', () => deleteAudio(filename, audioItemDiv));
    audioItemDiv.appendChild(delBtn);
...
function deleteAudio(wavPath, cardEl) {
        fetch('/delete_audio', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ wav: wavPath }) })
            .then(r=>r.json())
            .then(j=>{ if (j.success) { cardEl.remove(); } else { console.error('Удаление не удалось', j.error); } })
