...
### Лицензирование
- Код этого репозитория: публичное достояние (Unlicense).
- Модели / веса (MusicGen, MultiBandDiffusion и др.) подчиняются лицензиям их владельцев (Meta и др.) и могут иметь ограничения.
- Сгенерированное аудио может подпадать под условия upstream лицензии; вы сами отвечаете за clearance перед коммерческим использованием.
- См. `LICENSE` (addendum) и `LICENSE_MODELS.md`.
